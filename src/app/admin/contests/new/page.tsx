...
export default function CreateContestPage() {
  ...
  const sectionTitle = (title: string) => (
    <h2 className="text-base font-semibold mb-4 pb-3" style={{ borderBottom: '1px solid var(--border)' }}>
      {title}
    </h2>
  )
      ...
      <main className="flex-1 ml-60 overflow-y-auto p-8">
        <div className="max-w-3xl mx-auto">
          <div className="mb-8">
            ...
            <p style={{ color: 'var(--text-secondary)' }}>Set up a timed assessment for your students</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Rules (optional)</label>
                  <textarea value={form.rules} onChange={set('rules')} rows={3}
            ...
            <div className="glass rounded-2xl p-6">
              ...
              {selectedProblems.length > 0 && (
                <div className="mb-4 space-y-2">
                  ...
                  {selectedProblems.map((p, i) => {
                    ...
                    return (
                      <div key={p.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-md" style={{ color: diff.color, background: diff.bg }}>
                          {diff.label}
                        </span>
                        <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{p.points} pts</span>
                        <button type="button" onClick={() => toggleProblem(p)}
                  ...
                  <div className="text-right text-sm pt-1" style={{ color: 'var(--text-muted)' }}>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {filtered.length === 0 && (
                  <p className="text-sm text-center py-4" style={{ color: 'var(--text-muted)' }}>
                    {problems.length === 0 ? 'Loading problems…' : 'No problems found'}
                  </p>
                )}
                {filtered.map(p => {
                  const diff = DIFFICULTY_CONFIG[p.difficulty]
                  return (
                    <button key={p.id} type="button" onClick={() => toggleProblem(p)}
